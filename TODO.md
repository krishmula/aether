...
Add new dataclasses to `gossip_protocol.py` for subscriber ↔ broker communication.
...
Create `network_subscriber.py` - a network wrapper around the existing `Subscriber`.
...
Modify `gossip_broker.py` to track and deliver to remote subscribers.
...
| File | Action |
|------|--------|
| `gossip_protocol.py` | Add new message dataclasses |
| `network_subscriber.py` | Create new file |
| `gossip_broker.py` | Extend for remote subscriber tracking |
| `tests/test_network_subscriber.py` | Create new test file |

---

## Notes

- `broker.py` stays unchanged (handles local subscribers only)
- `subscriber.py` stays unchanged (core logic)
- `GossipBroker` handles both local (`Broker`) and remote (new tracking) subscribers
